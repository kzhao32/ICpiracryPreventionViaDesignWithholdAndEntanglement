...
public class InputsCount2BenchToDOTDigraph {

	public static void main(String[] args) {
		InputsCount2BenchToDOTDigraph instance = new InputsCount2BenchToDOTDigraph();
		instance.start(args);
	}
	...
	public void start(String[] args) {
		...
		{
			int totalNumberOfAllGateInputs = 0;
			int totalTransistorCount = 0;
			String inputFileName = inputFileNames[q];//"cQuiz2.f.f.f.0.bench";//inputFileNames[0]; //inputFileNames2[1];
				...
				{							// keep reading lines to compute the number of agents
					...
					if (stringWalker.length()>0)
					{
						...
						else //gate
						{
							...
							String stringWalker2 = stringWalker;
							...
							int numInputsToThisGate=0;
							while (stringWalker.contains(","))
							{
								totalNumberOfAllGateInputs++;
								numInputsToThisGate++;
							...
							if (stringWalker2.contains("NAND"))
							{
								totalTransistorCount += numInputsToThisGate * 2;
							}
							else if (stringWalker2.contains("AND"))
							{
								totalTransistorCount += numInputsToThisGate * 2 + 2;
							}
							else if (stringWalker2.contains("XNOR"))
							{
								totalTransistorCount += 8;
							}
							else if (stringWalker2.contains("NOR"))
							{
								totalTransistorCount += numInputsToThisGate * 2;
							}
							else if (stringWalker2.contains("XOR"))
							{
								totalTransistorCount += 6;
							}
							else if (stringWalker2.contains("OR"))
							{
								totalTransistorCount += numInputsToThisGate * 2 + 2;
							}
							else if (stringWalker2.contains("BUF"))
							{
								totalTransistorCount += 4;
							}
							else// if (stringWalker2.contains("NOT"))
							{
								totalTransistorCount += 2;
							}
							if (containsCluster > 0) {
			...
			//    	System.out.println(inputFileNames[q] + "; numGateInputs="+totalNumberOfAllGateInputs+"; numTransistors="+totalTransistorCount);
			System.out.print(inputFileNames[q].substring(0, inputFileNames[q].indexOf(".bench")) + ";"+totalNumberOfAllGateInputs+";"+totalTransistorCount);
			double[] hwCost = {0.025,0.05,0.075,0.1,0.125,0.15, 0.175, 0.2,0.225,0.25};
			double maxN, maxK;
			// n is number of inputs in 11st layer
			//
			double maxHWcost, maxNumPuzzles;
			maxN = maxK = 0;
			maxHWcost = maxNumPuzzles = 0;
			for (int i = 0; i < hwCost.length; i++) {
				double hwCeiling = totalTransistorCount*hwCost[i];	// max number of transister we can withhold // OLD change from int to double
				//    		System.out.println("\t" + hwCost[i]+"= " +hwCeiling);
				for (double n = 1; n < 20; n+=0.01) {
					for (double k = 1; k < 20; k+=0.01) {
//						if (k*(long)Math.pow(2, n)+(long)Math.pow(2, k) <= hwCeiling) {	// OLD
						if ((6*k*(((long)Math.pow(2, n))-1)+((long)Math.pow(2, k))-1) + (1*k*((long)Math.pow(2, n))+((long)Math.pow(2, k))) <= hwCeiling) {	// OLD
							// number of T for muxes                                           // number of Ts for cells
							if (Math.pow(2, n*k) > maxNumPuzzles) {
								maxN = n;
								maxK = k;
//								maxHWcost = k*Math.pow(2, n)+Math.pow(2, k); //k*(2^n)+(2^k)
								maxNumPuzzles = Math.pow(2, n*k); // 2^(n*k)
							}
						} else {
							break;
						}
				...
				//    		System.out.println("\t" + hwCost[i]+"= " +hwCeiling+"; n= "+maxN+"; maxK= "+maxK+"; maxHWcost= "+maxHWcost+"; maxNumPuzzles="+maxNumPuzzles);
				//    		System.out.print(";"+hwCost[i]+";"+hwCeiling+";"+maxN+";"+maxK+";"+maxHWcost+";"+maxNumPuzzles);
				System.out.print(";"+maxNumPuzzles);

			}
			System.out.println();


			//numGateInputs="+totalNumberOfAllGateInputs+"; numTransistors="+totalTransistorCount);

			//    	+"; 2.5%="+(int)(totalTransistorCount*0.025)+"; 5%="+(int)(totalTransistorCount*0.05)+"; 7.5%="+(int)(totalTransistorCount*0.075)+"; 10%="+(int)(totalTransistorCount*0.1)+"; 12.5%="+(int)(totalTransistorCount*0.125)+"; 15%="+(int)(totalTransistorCount*0.15));
			//    	double[] hwCost = {0.025,0.05,0.075,0.1,0.125,0.15};
			//    	for (int)
		}
	}
