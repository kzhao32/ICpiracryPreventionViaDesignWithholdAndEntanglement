...
public class InputsCount {
	...
	public static void main(String[] args) {
		InputsCount instance = new InputsCount();
		instance.start(args);
	}
	...
	public void start(String[] args) {
		...
		String inputFileName = inputFileNames[0];//inputFileNames[0];
							...
							for (int x = 0; x < numberOfGates; x++) {
								if (!gateInputs[x].arrayList.contains(l)) {
									for (String o : gateInputs[x].arrayList) {	// for ever current input
										if (m.equals(o)) {								// if l is an input
											gateInputs[x].arrayList.add(l); // then add m
											isDone = false;
											break;
										}
									}
								...
								if (!gateOutputs[x].arrayList.contains(m)) {
									for (String o : gateOutputs[x].arrayList) {	// for ever current input
										if (l.equals(o)) {								// if l is an input
